...
int SpredisSTempBase_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, SpredisSetCont * (*command)(SpredisSetCont **, int), int stripEmpties, int includeStore) {
    ...
    RedisModuleKey **keys = RedisModule_PoolAlloc(ctx, sizeof(RedisModuleKey*) * (keyCount + includeStore));
    SpredisSetCont **sets =  RedisModule_PoolAlloc(ctx, sizeof(RedisModuleKey*) * (keyCount + includeStore));
