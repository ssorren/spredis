...
module.exports = class Indexer extends Prefixed {
	...
	async  getDocuments(ids) {
		...
		let res;
		try {
			res = await qs.callBuffer('spredis.records', this.prefix, ...ids);
		} catch (e) {
			throw e;
		} finally {
	...
	async  getDocument(id) {
		let res = await this.getDocuments(id);
	...
	async indexDocuments(docs, full) {
		...
		try {
			let pipe = this.conn.pipeline();
			let command;
			let docBody;
			for (var i = 0; i < docs.length; i++) {
				let doc = docs[i];
				if (doc) {
					if (!doc.id) throw new Error('No id for document!:' + (doc));	
				}
				let fieldCount = 0;
				command = [];
				docBody = JSON.stringify(doc);
				doc = this.applyFieldValues(doc);

				_.forEach(this.allStrategies, (strat, key) => {
					let ir = strat.indexArguments(doc[key]);
					if (ir) {
						fieldCount++;
						command.push(...ir);
					}
				...
				// let oLen = docBody.length;
				// let compressed = lz4.encode(docBody);
				// console.log(`commpressed: ${compressed.length}, original: ${docBody.length}, ratio: ${compressed.length / docBody.length}`)
				command.push(docBody);
				// command.push(compressed, oLen);
				// console.log(compressed.toString());
				pipe.call('spredis.addrecord', this.prefix, doc.id, fieldCount, ...command);
			}
			if (pipe.length) {
				let reses = await pipe.exec();
				// console.log(reses);
				for (var i = 0; i < reses.length; i++) {
					if (reses[i][0]) {
						console.log(pipe._queue[i+1], reses[i][0]);
					}
				}
			}	
		} catch(e) {
		...
		// 		if (def.typeDef.index || def.typeDef.facet) {
	...
	async unIndexDocuments(docs, full) {		
		...
		try {
			...
			for (var i = 0; i < docs.length; i++) {
				...
				pipe.call('spredis.deleterecord', this.prefix, doc.id);
			}
			if (pipe.length) {
				let reses = await pipe.exec();
				// console.log(reses);
