...
module.exports = class Indexer extends Prefixed {
		...
		try {
			...
			for (let i = 0; i < docs.length; i++) {
				...
				if (oldDoc) {
					...
					oldVal = oldVal ? (def.typeDef.array && _.isArray(oldVal) ? oldVal : [oldVal]) : []
				...
				val = val ? (def.typeDef.array && _.isArray(val) ? val : [val]) : [];
	...
	async  reIndex(full) {
		...
		// let self = this;
		// let stream = conn.zscanStream(this.allIdsKey, {
		// 	count:100
		// })
		// let tempKey = this.getTempIndexName();
		// let start = new Date().getTime();
		// await conn.sort(this.allIdsKey, 'BY', 'nosort', 'GET', '#', 'STORE', tempKey);
		// let end = new Date().getTime();
		// console.log(`Sort allIds took ${end - start}ms`);
		// conn.expire(tempKey, 10000);
		// try {
		// 	let processed = 0;
		// 	let len = await conn.llen(tempKey);
		// 	console.log(len)
		// 	//procees the docs 100 at a time
		// 	while (len > 0) {
		// 		let sample = await conn.lrange(tempKey, 0, 99);
		// 		processed += sample.length;
		// 		console.log('sample length:', sample.length)
		// 		await conn.ltrim(tempKey, 100, -1);
		// 		len = await conn.llen(tempKey);

		// 		// len -= processed;
		// 		let docs = []
		// 		for (var i = 0; i < sample.length; i++) {
		// 			let id = sample[i];
		// 			let doc = await conn.hgetBuffer(this.valueKey, id);
		// 			doc = JSON.parse(unpack(doc).toString());
		// 			docs.push(doc);
		// 		}
		// 		await this.indexDocuments(docs);
		// 		console.log('docs:', docs.length)
		// 		console.log('remaining:', len)
		// 		await conn.expire(tempKey, 10000);
		// 		// break;
		// 	}
		// } catch (e) {
		// 	console.log(e.stack)
		...
		// console.log('tempKey', tempKey, _.isNumber(len), sample);
		...
		// 				await conn.sadd(tempKey, )
		// 				let doc = JSON.parse(unpack(data[i + 1]).toString());
		// 				docs.push(doc);
		// 			}
		// 			await self.indexDocuments(docs, full);
		// 			total += (data.length / 2)
	...
	async indexDocuments(docs, full) {
		...
		try {
			...
			for (var i = docs.length - 1; i >= 0; i--) {
				...
				// console.log('A:',docs[i]._lid, docs[i]._id, docs[i].id)
			}
			...
			if (!full) {
				...
				_.each(docs, (doc) =>{
					// console.log('getting old', doc._id)
			...
			for (var i = docs.length - 1; i >= 0; i--) { //validate id and store in all ids
				...
				pipe.call('spredis.sadd', this.allIdsKey, doc._id);

				// console.log('B:',doc._lid, doc._id)
				delete doc._lid;
				// if (oldDocs[i]) {
		...
		try {
			...
			for (var i = docs.length - 1; i >= 0; i--) {
				if (nres[i][1]){
					...
					docs[i]._lid = theId;
					docs[i]._id = utils.longToID( theId );
				}
			}
