...
module.exports = class Indexer {
	...
	async  getDocument(id) {
		let nid = await this.conn.existingId(this.idMap, id);
		if (!nid) throw new Error(`Could not find document for ${id}`);
		let res = await this.conn.getBuffer(this.getValueKey(  utils.longToID( parseInt(nid) ) ));
	...
	async indexDocuments(docs, full) {
		...
		try {
			...
			if (!full) {
				...
				_.each(docs, (doc) =>{
					readPipe.getBuffer( this.getValueKey(doc._id) );
				});
			...
			for (var i = docs.length - 1; i >= 0; i--) { //validate id and store in all ids
				...
				pipe.set( this.getValueKey(doc._id), pack( JSON.stringify(doc) ) );
