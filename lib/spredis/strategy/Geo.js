...
module.exports = class GeoStrategy extends Range {
	...
	setForSingleValue(pipe, val, lang, hint, cleanUp) {
		...
		if (val.radiusField) {
			...
			radiusField = rStrat.indexForValue(null, lang);
		} else if (!query.radius) {
			throw new Error(`(geo:search) Geo searches must have a radius or a radiusField`);
		}

		if (radiusField) {
			pipe.call('spredis.storerangebyradius', store, indexName, hint || '', query.from[0], query.from[1], 1, query.unit, radiusField);
		} else {
			pipe.call('spredis.storerangebyradius', store, indexName, hint || '', query.from[0], query.from[1], query.radius, query.unit);	
		}
	...
	async unIndexField(pipe, id, val, pos) {
		...
		if (exists) {
			...
			// pipe.zrem(indexName + '_r', this.toLex(id, pos));
	...
	async indexField(pipe, id, val, pos) {
		...
		if (exists) {
			...
			// pipe.geoadd(indexName + '_r', rank[1], rank[0], this.toLex(id, pos));
		}
