	...
	unIndexDocument(pipe, doc) {
		...
		for (let i = 0; i < perms.length; i++) {
			...
			if (this.fieldsSatisfiedBy(perm)) {
				let s = this.lexValue(perm, this.lang);
				let indexName = this.indexForValue(perm, this.lang);
				let id = this.toLex(doc._id);
				// console.log('del perm:', id, perm);
				// pipe.call('spredis.srem', indexName ,id);
	...
	indexDocument(pipe, doc) {
		...
		for (var i = 0; i < perms.length; i++) {
			...
			if (this.fieldsSatisfiedBy(perm)) {
				...
				let id = this.toLex(doc._id);
				// console.log('add perm:', id ,perm);
				pipe.call('spredis.zladd', indexName, id, 0, s);
				// pipe.call('spredis.sadd', indexName, id);
			}
		}
