...
module.exports = class TextStrategy extends Literal {
	...
	indexArguments(tokens, lang='en') {
		if (!tokens) return null;
		if (_.isArray(tokens) && !tokens.length) return null;
		tokens = _.uniq(textUtils.filter(tokens, lang, this.type.stopWords));
		_.remove(tokens, (a) => {
			return a === '';
		});
		return super.indexArguments(tokens);
	}
