...
module.exports = class LiteralStrategy extends Strategy {
	...
	indexArguments(val) {
		if (this._exists(val)) {
			...
			let command = [this.name, val.length];
			for (var i = 0; i < val.length; i++) {

				let indexVal = clean(val[i]);
				indexVal = this.type.stem ? textUtils.stem(indexVal, lang) : indexVal;
				command.push(indexVal, this.actualValue(val[i]));
			}
			return command;
		}
		return null;
	...
	_lexSearch(pipe, store, index, hint, value) {
		...
		pipe.call('spredis.storerangebylex', this.prefix, this.name, store, hint, start, stop);
	}
	...
	indexForSpecificValue(pipe, val, lang, cleanUp) {
		...
		val = this.type.stem ? textUtils.stem(val, lang) : val;
		pipe.call('spredis.zllinkset', this.prefix, this.name, store, val);
		return store;
	}
