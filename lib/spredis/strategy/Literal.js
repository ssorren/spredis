...
module.exports = class LiteralStrategy extends Strategy {
	...
	setForSingleValue(pipe, val, lang, hint, cleanUp) {
		...
		if (pWildCardIndex && sWildCardIndex) {
			...
			cleanUp.push(t);
			pipe.szunionstore(t, 2, pWildCardIndex, sWildCardIndex)
			return t;
		}
		...
		if (hint) {
			...
			pipe.szinterstore(store, 2, hint, this.indexForSpecificValue(val, lang), 'WEIGHTS', 0, 0);
	...
	unIndexField(pipe, id, val, pos) {
		...
		if (exists) {
			...
			if (specIndexName) pipe.srem(specIndexName, this.toLex(id, pos));
			pipe.hdel(this.facetName(this.lang), id, this.actualValue(val));
	...
	indexField(pipe, id, val, pos) {
		...
		if (exists) {
			...
			if (specIndexName) {
				pipe.sadd(specIndexName, this.toLex(id, pos));
			} else {
				pipe.zadd(this.indexForValue(val, this.lang), rank, this.toLex(id, pos));	
			}

			if (this.type.facet) {
				pipe.hset(this.facetName(this.lang), id, this.actualValue(val));
			}
