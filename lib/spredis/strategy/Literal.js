...
module.exports = class LiteralStrategy extends Strategy {
	...
	indexForValue(val, lang) {
		return this.indexName + ':V:' + (this.type.universal ? 'U' : lang);
	}
	...
	unIndexField(pipe, id, val, pos, lang='en') {
		...
		if (exists) {
			...
			pipe.call('spredis.zlrem', this.indexForValue(val, lang), id, clean(val));
	...
	indexField(pipe, id, val, pos, lang='en') {
		...
		if (this._exists(val)) {
			...
			pipe.call('spredis.zladd', this.indexForValue(val, lang), id, doSort, clean(val)); 
