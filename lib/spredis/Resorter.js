...
class Resorter extends Prefixed {
	...
	async fullResort(source, finalSort, unsorted) {
		...
		await this.conn.sort(source, 'BY', '#', 'store', tempStore);
		console.log(`Sort values took ${this._timeIt(s)}ms`);

		let chunk = await this.conn.lrange(tempStore, 0, 99);
		s = new Date().getTime();
		let count = 0;
		while (chunk && chunk.length) {
			let command = [finalSort, 'XX'];
			for (var i = 0; i < chunk.length; i++) {
				...
				if (res) {
					...
					// res = res.match(/:([^:]+)/);
					if (res && res[1]) {
						command.push(++count, res[1]);
					}
				}
			}
			if (command.length) {
				this.conn.zadd.apply(this.conn, command);
			}
			await this.conn.ltrim(tempStore, 100, -1);
			chunk = await this.conn.lrange(tempStore, 0, 99);
		}
