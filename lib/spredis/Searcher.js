...
module.exports = class Searcher extends Prefixed {
	...
	individualTextQuery(pipe, crit, fields, cleanUp, hint) {
		...
		if (indeces.length > 1) {
			...
			pipe.zunionstore.apply(pipe, com);
			return store;
	...
	compoundPhase(pipe, compoundIndeces, cleanUp, hint) {
		...
		pipe.zinterstore.apply(pipe, indeces);
		return store;
	}
	...
	applySort(pipe, indexName, sort, lang, start, count, cleanUp) {
		...
		// if (sort.length > 1) {
		// let s = pipe.createBuiltinCommand('spredis.sort');
		// let mi = [primarySort, finalSort, start, count];
		let mi = [indexName, finalSort, start, count];
		...
		pipe.spredissort.apply(pipe, mi);	
		// } else {
		// 	pipe.sort(primarySort, 'by', 'nosort', 'get', '#', 'limit', start, count, 'store', finalSort);
		// }
		...
		for (var i = 0; i < clause.children.length; i++) {
			...
			if (ci.index) {
				...
				if (ci.type === NOT) {
					...
					pipe.zinterstore(store, 2, clause.index, ci.index);
				}
				clause.index = store;
			}
		}
	...
	async search(request) {
		...
		if (multiSort) {
			indexName = this.applySort(pipe, indexName, sort, lang, start, count, cleanUp);
		}
		...
		if (request.facets) {
			this.buildFacets(pipe, request.facets, facetIndex, lang, cleanUp);
		}
