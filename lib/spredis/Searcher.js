...
module.exports = class Searcher {
	...
	distancExpression(pipe, expr, idSet, validExpressions, cleanUp) {
		let strat = this.getStrategy(expr.val.field);
		if (strat) {
	...
	assignExpressionValues(items, validExpressions, res) {
		let idMap = {};
	...
	async search(request) {
		...
		*/
				...
				if (expressions.length) {
					...
					for (let i = 0; i < expressions.length; i++) {
						...
						if (e.val.type === 'distance') {
							this.distancExpression(pipe, e, exprSet, validExpressions, cleanUp)
						}
					}
				} else {
					getItemsIndex = pipe.length;
		...
		let response = await this.executeSearch(pipe, request, validExpressions, { 
					countResultindex:countResultindex, 
					memberResultindex:memberResultindex,
					getItemsIndex:getItemsIndex
				}, prepEndTime - prepStartTime, count, idsOnly, exprIds, start);
	...
	async executeSearch(pipe, request, validExpressions, resultPointers, prepTime, count, idsOnly, exprIds, start) {
		...
		let totalFound;
		totalFound = res[resultPointers.countResultindex][1];
		let itemCount = 0;
		let items = null;
		// let exprStartTime = 0,
		// 	exprEndTime = 0;
		if (count === 0 || totalFound === 0) {
			...
			items = res[resultPointers.memberResultindex][1];
			itemCount = items.length;
		} else {
			
			// console.log(res[getItemsIndex][1][0])
			items = _.map(res[resultPointers.getItemsIndex][1], (i) =>{ return JSON.parse(unpack(i).toString());});
			

			itemCount = items.length;
			if (exprIds && validExpressions.length) {
				this.assignExpressionValues(items, validExpressions, res);
			}
