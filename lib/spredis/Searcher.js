...
module.exports = class Searcher {
	constructor(prefix, defs, compounds) {
		...
		this.valueKeyPattern = prefix + ':ID:*';
		...
		*/
		...
		if (expr.length) {
			_.forEach(query[expr[0]], (val, key) => {
				expressions.push({name: key, val: val});
			});	
		}
		...
		let literalIndex = this.performPhase(pipe, literalStrats, cleanUp, compoundIndex, false);
		 ...
		 */
		let geoIndex = this.performPhase(pipe, geoStrats, cleanUp, literalDeferred.index);
		...
		if (request.facets) {
			let self = this;
			let fcount = 0;
			facetResultIndex = pipe.length;
			_.each(request.facets, (value, key) => {
				...
				let tempList = self.getTempIndexName();
				let order = (value.order ? value.order.toUpperCase() : null) || 'ASC';
				let count = value.count || 10;
				pipe.facet(indexName, strat.facetPattern(lang), value.resultIndex, tempList, order, count);
				cleanUp.push(tempList, value.resultIndex);
		...
		let totalFound;
		totalFound = res[countResultindex][1];
		let itemCount = 0;
		let items = null;
		let exprStartTime = 0,
			exprEndTime = 0;
		if (count === 0 || totalFound === 0) {
			...
			items = res[memberResultindex][1];
			itemCount = items.length;
		} else {
			
			// console.log(res[getItemsIndex][1][0])
			items = _.map(res[getItemsIndex][1], (i) =>{ return JSON.parse(unpack(i).toString());});
			endSerialize = new Date().getTime();

			itemCount = items.length;
			if (exprIds && expressions.length) {
				// console.log(`evaluating expressions: ${JSON.stringify(expressions)}`);
				exprStartTime =  new Date().getTime();
				pipe = myConn.pipeline();
				let validExpressions = [];
				for (let i = 0; i < expressions.length; i++) {
					let e = expressions[i];
					if (e.val.type === 'distance') {
						let strat = this.getStrategy(e.val.field);
						if (strat) {
							let distanceIndex = strat.indexForValue(e.val.from);
							let query = strat.getQueryValue(e.val);
							let store = this.getTempIndexName();
							let exprSet = this.getTempIndexName();
							cleanUp.push(store, exprSet);
							pipe.convertToSet(exprIds,exprSet);
							pipe.zinterstoreBuffer(store, 2, exprSet, distanceIndex, 'WEIGHTS', 0, 1);

							e.resultIndex = pipe.length;
							validExpressions.push(e);
							pipe.georadius(store, query.from[1], query.from[0], '+inf', query.unit, 'WITHDIST')
						}
					}
				}
				
				let exprResults = await pipe.exec();
				let idMap = {};
				...
				for (let i = 0; i < validExpressions.length; i++) {
					let ex = validExpressions[i];
					if (ex.val.type === 'distance'){
						let eRes = exprResults[ ex.resultIndex ][1];
				...
				exprEndTime =  new Date().getTime();
			}
			
			// itemCount = res[getItemsIndex][1];
		}
		...
		let response = {
			...
			count: itemCount,
			facets: {},
			// items: idsOnly ? items : undefined,
			items: items,
			// handle: items === null ? sortedResultsList : undefined, 
			prepTimeMillis: prepEndTime - prepStartTime,
			queryTimeMillis: endTime - startTime,
			exprTimeMillis: exprEndTime - exprStartTime,
		...
		_.each(request.facets, (value, key) => {
			...
			response.facets[key] = o;
		});

		// if (response.handle) {
		// 	let t = await  this.redis.object('encoding',response.handle);
		// 	console.log(t);
		// }

		// let sortStart = new Date().getTime();
		// let self = this;
		// this.redis.sort(indexName, 'BY', 'nosort', 'GET', '#', 'STORE', 'temb').then( (res) => {
		// 	let ellapsed = new Date().getTime() - sortStart;
		// 	console.log(`Sorted entire set in ${ellapsed}ms`);
		// 	self.redis.del('temb');
