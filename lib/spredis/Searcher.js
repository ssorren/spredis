...
module.exports = class Searcher {
	...
	assignExpressionValues(items, expressions, res) {
		let idMap = {};
	...
	async executeSearch(pipe, opts) {
		...
		if (opts.count === 0 || totalFound === 0) {
			...
			items = _.map(res[opts.getItemsIndex][1], (i) =>{ return JSON.parse(unpack(i).toString());});
			itemCount = items.length;
			if (opts.exprIds && opts.expressions.length) {
				this.assignExpressionValues(items, opts.expressions, res);
			}			
		}
