...
module.exports = class Searcher extends Prefixed {
	...
	compoundPhase(pipe, query, lang, cleanUp) {
		...
		for (var i = 0; i < this.compounds.length; i++) {
			...
			if (satisfied && satisfied.length) {
				...
				if (permutaions.length === 1) {
					...
					// 	command.push(0);
					// }
					// pipe.zunionstore.apply(pipe, command);
		...
		if (compoundIndeces.length === 1) {
			...
			// }
			// pipe.zinterstore.apply(pipe, command);
			...
			pipe.sinterstore.apply(pipe, command);
			compoundIndex = temp;
		}
	...
	async search(request) {
		...
		*/
		...
		// console.log('range hint = ', textIndex.index);
		let rangeIndex = this.performPhase(pipe, rankStrats, cleanUp, textIndex.index, true);
		// let rangeIndex = this.performPhase(pipe, rankStrats, cleanUp, null, true);
		// if (rangeIndex && textIndex && rangeIndex.index && textIndex.index) {
		// 	// may be much faster if we don't pass a hint to the range indeces,
		// 	// this is bacause you may be starting off with 2 really large sets,
		// 	// in which case, the initial instersect would take a really long time
		// 	let rangeInersect = this.getTempIndexName(cleanUp);
		// 	pipe.sinterstore(rangeInersect, rangeIndex.index, textIndex.index)
		// 	rangeIndex = {index: rangeInersect, defer: null}
		// }
	...
	async executeSearch(pipe, opts) {
		...
		_.each(opts.request.facets, (value, key) => {
			...
			if (value.facetIndex >= 0) {
				// console.log(res[value.pipeIndex])
