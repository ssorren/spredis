...
module.exports = class Searcher extends Prefixed {
	...
	createCursor(pipe, indexName, sort, lang='en', start=0, count=0, idsOnly=0, expressions=[], cleanUp) {
		...
		if (expressions && expressions.length) {
			...
			for (var i = 0; i < expressions.length; i++) {
				...
				if (strat) {
					...
					if (!expr.resolver) {
						// let exprIndex = this.getTempIndexName(cleanUp);
						expr.resolverIndex = expCount++;
						mi.push(1, strat.name, query.from[0], query.from[1], query.unit);
						// pipe.call('spredis.setgeoresolver', exprIndex, expr.strategy.sortIndex(lang), 1, query.from[0], query.from[1], query.unit);
					}
					expr.resultIndex = pipe.length;
					// pipe.call('spredis.resolveexpr', resultSet, expr.resolver, start, count);
		...
		pipe.callBuffer('spredis.preparecursor', this.prefix, finalSort, indexName, lang, sortCount, expCount, start, count, idsOnly ? 1 : 0, ...mi);
		return finalSort;
	}
	...
	async search(request) {
		...
		try {
			...
			countResultindex = pipe.length;
			let cursorName = this.createCursor(pipe, indexName, sort, lang, start, count, idsOnly, expressions, cleanUp);
