...
module.exports = class Searcher extends Prefixed {
	...
	applySort(pipe, indexName, sort, lang='en', start=0, count=0, idsOnly=0, cleanUp) {
	
		let finalSort = this.getCursorName();
		let mi = [];
		if (sort) {
			for (let i = 0; i < sort.length; i++) {
				if (!sort[i].expr) {
					...
					//TODO: lang support
					mi.push(0, strat.name, sort[i].order);
				} else {
					...
					mi.push(1, expr.strategy.name, sort[i].order, query.from[0], query.from[1]);
					// mi.push(exprSortIndex, sort[i].order);
				}
				
			}	
		}
		// console.log('spredis.preparecursor', this.prefix, finalSort, indexName, lang, sort ? sort.length : 0, start, count, ...mi);
		pipe.callBuffer('spredis.preparecursor', this.prefix, finalSort, indexName, lang, sort ? sort.length : 0, start, count, idsOnly ? 1 : 0, ...mi);
		return finalSort;
	}
	...
	buildFacets(pipe, facets, cursorName, lang, cleanUp) {
		if (!facets || !_.keys(facets).length) return;
		let self = this;
		let fcount = 0;

		let command = ['spredis.facets', this.prefix, cursorName];
		_.each(facets, (value, key) => {
			if (value.field) {
				...
				command.push(strat.name, count, order);
		...
		fcount = 0;
		let rcommand = ['spredis.rangefacets', this.prefix, cursorName];
		_.each(facets, (value, key) => {
			if (value.buckets) {
	...
	async search(request) {
		...
		try {
			...
			countResultindex = pipe.length;
			let cursorName = this.applySort(pipe, indexName, sort, lang, start, count, idsOnly, cleanUp);

			let exprSet = null;
			
			if (count > 0) {
				getItemsIndex = countResultindex;
				memberResultindex = getItemsIndex;
				if (expressions.length) {

					for (let i = 0; i < expressions.length; i++) {
						let e = expressions[i];
						if (e.val.type === 'distance') {
							this.distancExpression(pipe, e, cursorName, start, count, lang, cleanUp)
						}
					}
				}
				// if (!idsOnly) {
				// 	getItemsIndex = pipe.length;
				// 	pipe.callBuffer('spredis.getresdocs', this.prefix, cursorName, start, count);
				...
				// 	// pipe.call('spredis.getresids', this.prefix, indexName, start, count);
				// 	for (let i = 0; i < expressions.length; i++) {
				// 		let e = expressions[i];
				// 		if (e.val.type === 'distance') {
				// 			this.distancExpression(pipe, e, cursorName, start, count, lang, cleanUp)
			...
			if (request.facets) {
				this.buildFacets(pipe, request.facets, cursorName, lang, cleanUp);
			}
			...
			if (cleanUp.length) {
				myConn.call('spredis.deltempset', ...cleanUp).catch(e => {
					console.error(e.stack);
				});
			}
			if (cursorName) myConn.call('spredis.deletecursor', cursorName).catch(e => {
				console.error(e.stack);
			});
		} catch (ge) {
	...
	async executeSearch(pipe, opts) {
		...
		// console.log("pipe", JSON.stringify(pipe._queue, null, 2));
		...
		let totalFound;
		let cresult = res[opts.countResultindex][1];
		// console.log(JSON.stringify( cresult, null, 2 ));
		totalFound = cresult[0] || 0;
		let docsOrIds = cresult[1] || [];
		let itemCount = docsOrIds.length;
		let items = [];
		let expResult = {};
		let facetRes = {};
		...
		if (opts.count === 0 || totalFound === 0) {
			...
			let doc;
			if (raw) {
				for (let i = 0; i < docsOrIds.length; i++) {
					try {
						// doc =  docsOrIds[i].toString('utf8');
						items.push(docsOrIds[i].toString('utf8'));	
					} catch(e) {
						console.error(e.stack)
					}
				}
